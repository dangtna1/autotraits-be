...
def get_measurement(db: Session, measurement_id: int):
    return (
        db.query(PlantMeasurement).filter(PlantMeasurement.id == measurement_id).first()
    )


def get_measurements(
    db: Session, plant_id: Optional[str] = None, date: Optional[date] = None
):
...
def update_measurement(
    db: Session, measurement_id: int, update_data: MeasurementUpdate
):
